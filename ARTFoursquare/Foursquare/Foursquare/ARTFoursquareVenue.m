...
#import "KCFoursquareVenue.h"

@implementation KCFoursquareVenue
...
- (id)initWithData:(NSDictionary *)data {
    ...
    if (self) {
        ...
        for (NSDictionary *categoryData in [data valueForKey:@"category"]) {
            KCFoursquareCategory *category = [[KCFoursquareCategory alloc] initWithData:categoryData];
            [self.categories addObject:category];
            [category release];
        }
        
        NSDictionary *locationData = [data valueForKey:@"location"];
        self.location = [[[KCFoursquareLocation alloc] initWithData:locationData] autorelease];
    }
